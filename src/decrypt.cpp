...
void decrypt(const std::string& input_path, const std::string& output_path) {
    ...
    if (crypto_secretstream_xchacha20poly1305_init_pull(&crypto_state, header, key) != 0) throw KeyError("Invalid header or key");
    ...
    do {
        ...
        if (crypto_secretstream_xchacha20poly1305_pull(
            ...
            bytes_read,
            NULL, 0)
        != 0) throw UtilException("Decryption failed. The input file maybe corrupt");
